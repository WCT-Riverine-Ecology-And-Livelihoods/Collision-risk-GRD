...
distance <- 550000 #distance travelled by a boat on the 550 km of Hooghly in m
n.days <- 365 #number of days
n.boats <- 500 #number of boats
n.dolph <- 250 #number of dolphins
v.dolph <- 0.97222 #fixed speed (m/s) =0.972222 for ganges river dolphin based on  Sasaki-Yamamoto et al. (2012) - 3.5 km/hr
l_boat <- c(3, 10, 20) #length of boat (m)
w_boat <- c(1.5, 4, 8) #width of boat (m)
l_dolph <- rep(2.25, 3) #length of dolphin (m)
w_dolph <- rep(0.5, 3) #width of dolphin (m)
...
v.boat <- seq(1, 5, 0.1) #in m/s (based on vboat of 8-15 km/hr)
...
df_rc_radius <- df_rc_radius %>% 
  mutate(p_avoid = case_when(
    v.boat <= 2 ~ 0.95,
    v.boat > 2 & v.boat < 4 ~ 0.7,
    v.boat >=4 ~ 0.5
))
...
n_collisions <- c()
for(i in 1:nrow(df_rc_radius)){
 n_collisions[i] <- mean(rbinom(n_sim, 
                                size = as.integer(df_rc_radius$n_encounters[i]), 
                                p = p_striking_depth * (1 - df_rc_radius$p_avoid[i])))
}
...
ggplot(df_rc_radius, aes(x = v.boat, y = n_collisions, color = boat_type)) +
  geom_line() +
  geom_point() +
  scale_y_continuous(breaks = seq(from = 0, to = 33000, by = 5000)) +
